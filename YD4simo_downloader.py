...
#. --- NEW: ASCII icons and some colors added with another Features! ---
#.===============================================================
...
# ---------------------------------
# Print colored title before download
# ---------------------------------
def try_print_title(url: str, ffmpeg_bin: str = None):
    """
    Use YoutubeDL to extract and print the title.
    IMPORTANT: pass the full ffmpeg executable path via 'ffmpeg_location'
    to avoid 'ffmpeg not found' warnings during probing.
    """
    try:
        with YoutubeDL({
            "quiet": True,
            "no_warnings": True,           # suppress non-critical warnings
            "ffmpeg_location": ffmpeg_bin or "ffmpeg"
        }) as ydl:
...
def main():
    ...
    # Pass the full ffmpeg path to avoid 'ffmpeg not found' warnings here too
    try_print_title(url, ffmpeg_bin=ffmpeg_bin)
    ...
    ydl_opts = {
        ...
        # IMPORTANT: pass the full ffmpeg executable path (not its directory)
